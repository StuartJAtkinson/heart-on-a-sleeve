...
- [ ] **Saved-design cards show raw merch-type slugs instead of the friendly picker labels** — both `dashboard.html` (line 174: `` ${emoji} ${p.merch_type} ``) and the in-SPA "My Designs" panel (`frontend/cesium/src/app.ts`, `renderDesigns()`, same `${emoji} ${p.merch_type}` pattern ~line 1874) print the raw `merch_type` value (`tshirt`, `3d_print`, `tote`) instead of the Title-Case display names used in the merch-type picker in `index.html` (T-Shirt, Mug, Tote Bag, Coaster, Placemat, Relief). Reads as unfinished next to the rest of the polished UI. Add a shared `MERCH_LABEL` map (mirrors the existing `MERCH_EMOJI` map already duplicated in both files) and use it in both card templates.
- [ ] **In-SPA "My Designs" delete button has no accessible label, unlike its dashboard.html equivalent** — `frontend/cesium/src/app.ts`'s designs-panel delete button (`.design-del-btn`, ~line 1880) renders as a bare `✕` glyph with no `title`/`aria-label`, so a screen reader announces nothing useful and a first-time user has no text cue it deletes the design. `dashboard.html`'s equivalent (line 179) is a labelled `Delete` button for the same destructive action. Add `title="Delete design"` (or match dashboard's full-text label) to the in-SPA button.
