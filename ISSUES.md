...
- [ ] **Undefined CSS variable `--bg-base` in dashboard.html** — `frontend/cesium/public/dashboard.html` line 30-31 sets `#main { background: var(--bg-base); }`, but `--bg-base` is never defined in `app.css`'s `:root` (only `--bg-page`, `--bg-panel`, `--bg-item`, `--bg-hover` exist). It silently falls back to the property's initial value (transparent) instead of a panel/page background. Almost certainly meant `--bg-page` (used for the same "main content area" role elsewhere, e.g. `#svg-view`, `#viewer-3d-view`).
- [ ] **No shared danger/error/success colour tokens — two different hardcoded reds for the same semantic role** — `app.css`'s `:root` defines a full token system (bg/border/text/accent/toggle) but no `--color-danger`/`--color-error`/`--color-success`. Result: error text in `login.html` (lines 73, 208, 239, 414) hardcodes `#e07070`, while danger/logout styling in `dashboard.html` (lines 61, 64) and `index.html` (lines 153-154, 163-164, 260, 263) hardcodes a *different* red `#e06060` — the same "destructive/error" meaning rendered in two shades depending on file. `login.html` line 410 also hardcodes a one-off success green `#4a9a4a` with no token either. Add `--color-danger` / `--color-success` to `app.css` and point all of the above at them.
- [ ] **"Logout" vs "Sign out" — same action, different label per page** — `dashboard.html` line 103 and the in-SPA user nav (`frontend/cesium/src/app.ts`, `updateUserNav()`, ~line 1778: `↩ Logout`) both say "Logout", but `landing.html` line 112 says "Sign out" for the identical sign-out action. Standardize on "Logout" (majority usage) in `landing.html`.
