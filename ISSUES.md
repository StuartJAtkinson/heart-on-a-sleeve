...
- [ ] **Hardcoded accent hex `#3a7acc` for the active merch ratio label** — `frontend/cesium/index.html:50` sets `.merch-btn.active .ratio { color: #3a7acc; }` instead of using the existing `--accent` token (the rest of the project routes through `var(--accent)` — see `.btn.on`, `.btn.active`, `.swatch.active`, `.brand-row :focus`, etc.). The hex is also a *different* blue from `--accent` (`#3a7acc` vs the actual accent `#3a8eef` you'd find on `.btn-primary:hover`), so the active-ratio text reads slightly off-hue from the rest of the active state. Drop the inline hex and use `var(--accent)`. *(found 2026-08-17)*
- [ ] **Initial 3D-panel inline styles duplicate the shared `#svg-save-status` rule** — `frontend/cesium/index.html:364` (stl-save-status) and `:388` (print-save-status) repeat the same `style="font-size:11px;color:var(--text-mid);min-height:14px;margin-top:4px;"` that already has a CSS rule at `:173` for `#svg-save-status`. The SVG-side one was supposed to be the only CSS-rule version (issue "Duplicate `#svg-save-status` CSS rule" resolved 2026-08-02). Mixed conventions read as unfinished: `#svg-save-status` and `#designs-save-status` use CSS rules, `#stl-save-status`/`#print-save-status` use inline styles. Promote the inline styles to CSS rules (one per id, single rule, no inline). *(found 2026-08-17)*
- [ ] **`<div id="status-3d">`, `<div id="status-print">`, `<div id="stats-3d">`, `<div id="regen-status-print">` use inline styles** — `frontend/cesium/index.html:367-368, 391, 395` each repeat the same inline `font-size:11px;color:var(--text-mid);min-height:14px;margin-…` pattern. Same reasoning as the save-status row above: the rest of the repo uses id-keyed CSS rules for these (e.g. `#svg-status`, `#svg-regen-status`, `#svg-save-status`). Promote to CSS rules. *(found 2026-08-17)*
- [ ] **`index.html` `#loading-3d` has `display: none` but the print loader (`#loading-print`) has `display: flex`** — `frontend/cesium/index.html:183` sets `#loading-3d { display: none; }` while `:191` sets `#loading-print { display: flex; }`. Given the 2026-06-04 consolidation that moved loading to a single global `.app-status-bar` (status.ts), both loaders should be `display: none` (or removed entirely). One says "still wired in", the other says "removed" — pick one. *(found 2026-08-17)*
